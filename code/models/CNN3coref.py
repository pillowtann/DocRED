...
class CNN3coref(nn.Module):
	def __init__(self, config):
		super(CNN3coref, self).__init__()
		self.config = config
		...
		input_size = config.data_word_vec.shape[1] + config.entity_type_size #+ char_hidden
	...
	def forward(self, context_idxs, pos, context_ner, context_char_idxs, context_lens, h_mapping, t_mapping, relation_mask, dis_h_2_t, dis_t_2_h):
		...
		sent = torch.cat([self.word_emb(context_idxs), self.ner_emb(context_ner)], dim=-1)
